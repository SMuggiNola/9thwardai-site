// src/index.js — Math ACT Worker (with /math-act/* prefix)
// Students: /math-act/questions, /math-act/submit
// Admin: /math-act/debug/submissions, /math-act/debug/questions
...
export default {
  async fetch(request, env) {
    ...
    // Student routes
    if (path === "/math-act/questions" && request.method === "POST") {
      return await getQuestions(request, env);
    }
    if (path === "/math-act/submit" && request.method === "POST") {
      return await submitAnswers(request, env);
    }

    // Admin routes
    if (path === "/math-act/debug/submissions" && request.method === "GET") {
      if (!checkAdminKey(request, env)) return unauthorizedResponse();
      return await listSubmissions(url, env);
    }
    if (path === "/math-act/debug/questions" && request.method === "GET") {
      if (!checkAdminKey(request, env)) return unauthorizedResponse();
...
async function getQuestions(request, env) {
  ...
  if (latest) {
    ...
    if (latest.score >= 9 || attemptCount >= 3) {
      return await fetchBatch(latest.batch_id, env, 
        "You’ve already completed this set. A new one will be available next time."
      );
    } else {
      return await fetchBatch(latest.batch_id, env,
        "You’re still working on this set. You may submit again if needed."
      );
    }
  }
...
async function fetchBatch(batchId, env, message) {
  const ids = batchId.split("-").map((id) => Number(id));
...
// ------------------------------
// GET /math-act/debug/submissions
// ------------------------------
